...
description: "Production mechanics for Motion (formerly Framer Motion) in React and Next.js: correct imports, scroll-triggered reveals with whileInView, scroll-linked animation with useScroll, exit animations with AnimatePresence, MotionValues instead of re-render-per-frame, prefers-reduced-motion, and the performance rules that keep animation off the main thread. Use whenever the user asks to animate components, add transitions, create scroll-triggered or parallax effects, implement page transitions, layout animations, gesture or drag interactions, staggered reveals, or when code imports 'framer-motion', 'motion/react' or 'motion/react-client'. This skill covers how to build motion correctly and cheaply; for how motion should feel - timing, anticipation, follow-through, weight - the framer-motion skill (Disney's 12 principles) is the companion to it."
---
